/*
https://github.com/mochow13/competitive-programming-library/blob/master/Math/Faulhaber's%20Formula%20(Custom%20Algorithm).cpp
*/

#include<bits/stdc++.h>
#include <bits/stdtr1c++.h>

long long power(long long base, int exp, int MOD = 1e9+7){
    base %= MOD;
    if(exp == 0) return 1;
    if(exp % 2) return (base * power(base * base, exp / 2, MOD)) % MOD;
...
int main(){
    fool::init();
    int t, i, j;
    long long n, m, k, res, res1, res2, res3;

    cin >> t;
    while (t--){
        cin >> n >> m >>  k;
        res1 = fool::faulhaber(k, n + m);
        res2 = (2 * (fool::faulhaber(k, n + m - 1))) % MOD;
        res3 = fool::faulhaber(k, n + m - 2);
        cout << res1 << " " << res2 << " " << res3 << endl;
        res = (res1 + res3 - res2 + MOD) % MOD;
