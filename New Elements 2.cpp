...
ll bestY(ll a, ll b, ll c, ll d, ll l = 1, ll r = 2e9){
    if(l == r) return l;
    ll y = (l + r) / 2;
    ll x = (a * y) / b + 1;
...
int main(){
    ...
    for(int tt = 1; tt <= t; tt++){
        ...
        pair<ll, ll> minRatio = make_pair(0, 1);
        pair<ll, ll> maxRatio = make_pair(INF, 1);
        bool possible = true;
        for(int i = 0; i < n && possible; i++){
            for(int j = i + 1; j < n; j++){
                ...
                if(den > 0) maxRatio = selMin(maxRatio, rat);
                else minRatio = selMax(minRatio, rat);
            }
        }
        pair<ll, ll> mn = make_pair(0, 1);
        pair<ll, ll> mx = make_pair(INF, 1);
